  ...
  function DrawGauge({ value, endValue, limitValue, label }) {
    ...
    useEffect(() => {
      ...
      const animate = (timestamp) => {
        ...
        const animationProgress = Math.min(progress / animationDuration, 1); // Ensure animation stops after duration
  
        setAnimatedValue((value) => {
          const change = value + (startValue - value) * animationProgress;
          return change;
        });
  ...
  }, [value, startValue, animatedValue, animationDuration]);
    useEffect(() => {
    ...
    }, [animatedValue, endValue, limitValue]);
